...
const Floor3 = (() => {
  ...
  // --- DOM REFS ---
  let gaugeBar, pressureLabel, velocityLabel,
      alertBanner, statusLabel, sustainBar;
  ...
  function init() {
    ...
    document.getElementById('f3-tap-btn')?.addEventListener('pointerdown', handleTap);
  }
  ...
  function updateUI() {
    ...
    velocityLabel.className    = pct >= TARGET
  ...
  function complete() {
    ...
    setTimeout(() => {
      document.dispatchEvent(new CustomEvent('floor:complete', {
        detail: { floor: 3 }
      }));
    }, 1500);
  }
