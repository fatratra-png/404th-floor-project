...
export default function Floor2() {
  ...
  return (
    <Layout floorNumber={2} title="Access Terminal" subtitle="Keypad Entry">
      <div className="flex-1 flex items-center justify-center p-4 md:p-8 relative bg-[#0a0e14] overflow-hidden">
        {/* Elevator cab background */}
          ...
          <div className="absolute inset-0 opacity-[0.04]"
            style={{
              backgroundImage: `
                ...
                linear-gradient(0deg, #94a3b8 1px, transparent 1px)
              `,
              backgroundSize: '60px 60px',
            }}
          />
          {/* Top rail */}
          <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-b from-slate-700/30 to-transparent" />
          {/* Side rails */}
          <div className="absolute top-0 bottom-0 left-0 w-2 bg-gradient-to-r from-slate-700/20 to-transparent" />
          <div className="absolute top-0 bottom-0 right-0 w-2 bg-gradient-to-l from-slate-700/20 to-transparent" />
          {/* Floor indicator strip */}
          <div className="absolute top-6 left-1/2 -translate-x-1/2 bg-black/60 border border-slate-700/50 rounded px-4 py-1">
            <div className="flex items-center gap-2">
              <span className="text-primary font-mono text-xs font-bold tracking-wider">↓</span>
              <span className="text-slate-400 font-mono text-[10px]">FLOOR</span>
              <span className="text-primary font-mono text-sm font-bold">02</span>
            </div>
          </div>
          {/* Floor glow */}
          <div className="absolute bottom-0 left-1/4 right-1/4 h-20 bg-gradient-to-t from-primary/[0.03] to-transparent" />
        </div>

        {/* Sticky Note Clue */}
        <div
          className={`absolute top-[12%] right-[8%] z-30 transition-all duration-500 ${
            ...
            <div className="p-6 flex flex-col gap-4">
                ...
                {keys.flat().map(key => (
                  <button
                    ...
                    className={`aspect-[4/3] rounded-lg flex items-center justify-center font-bold text-lg active:scale-95 transition-all ${
                      key === 'backspace'
                        ? 'bg-[#0d111a] border border-red-900/30 hover:bg-red-900/20 text-red-400'
                        : key === 'enter'
                          ? 'bg-primary border border-primary shadow-[0_0_15px_rgba(19,91,236,0.4)] text-white'
                          : 'bg-[#0d111a] border border-slate-700/50 hover:bg-[#1a2235] hover:border-primary/50 text-white'
                    }`}
                  >
                    {key === 'backspace' ? (
              ...
              <button
                onClick={() => {
                  ...
                  if (!diagnostic) Sounds.play('keyclick')
                }}
                className={`w-full py-2.5 font-bold rounded-lg border flex items-center justify-center gap-2 transition-all text-sm ${
                  diagnostic
                    ? 'bg-yellow-500 text-black border-yellow-500'
                    : 'bg-[#0d111a] hover:bg-[#1a2235] text-slate-300 border-slate-700'
                }`}
              >
