...
fuses.forEach(fuse => {
  fuse.addEventListener("dragstart", () => { dragged = fuse; });
});
...
function updateVoltage() {
  ...
  if (percent === 100) {
    ...
    if (statusEl) { statusEl.textContent = "ONLINE"; statusEl.classList.add("text-green-500", "font-bold"); }
    const criticalEl = document.getElementById("critical");
    if (criticalEl) { criticalEl.textContent = "System Online"; criticalEl.classList.remove("text-accent-red"); criticalEl.classList.add("text-green-500"); }
  }
}
...
dropZones.forEach(zone => {
  zone.addEventListener("dragover", e => {
    e.preventDefault();
    zone.classList.add("border-primary", "bg-slate-900/80", "shadow-[0_0_20px_rgba(19,91,236,0.3)]");
  });

  zone.addEventListener("dragleave", () => {
    zone.classList.remove("border-primary", "bg-slate-900/80", "shadow-[0_0_20px_rgba(19,91,236,0.3)]");
  });

  zone.addEventListener("drop", e => {
    e.preventDefault();
    if (!dragged) return;
    if (zone.dataset.filled === "true") { dragged = null; return; }

    const acceptedType = zone.dataset.accept;
    const fuseType = [...dragged.classList].find(cls => ["quantum","plasma","thermal","typeA"].includes(cls));
    if (fuseType !== acceptedType) { dragged = null; return; }

    dragged.remove();
    zone.innerHTML = "";

    let bgColor = "", iconColor = "", iconName = "blur_on";
    if (fuseType === "plasma")  { bgColor = "bg-primary/10";       iconColor = "text-primary"; }
    if (fuseType === "quantum") { bgColor = "bg-purple-500/10";    iconColor = "text-purple-400"; }
    if (fuseType === "thermal") { bgColor = "bg-accent-yellow/10"; iconColor = "text-accent-yellow"; }

    const fuseBox = document.createElement("div");
    fuseBox.className = "w-24 h-32 bg-gradient-to-br from-slate-700 to-slate-900 rounded border border-slate-600 flex items-center justify-center relative overflow-hidden";
    ...
    if (node) {
      node.querySelector("div").className = "w-2 h-2 rounded-full bg-primary animate-pulse shadow-[0_0_5px_#135bec]";
      node.querySelector("span").className = "text-xs text-primary font-bold font-mono";
    }
    if (indicators[index]) {
...
toggleBtn.addEventListener("click", () => { inventory.classList.toggle("hidden"); });
...
rebootButton.addEventListener('click', async () => {
  ...
  const nodes = [document.querySelector('.NODE_A'), document.querySelector('.NODE_B'), document.querySelector('.NODE_C')];
  nodes.forEach(node => {
    node.querySelector('div').classList.add('bg-primary','animate-pulse','shadow-[0_0_5px_#135bec]');
  ...
  accessMsg.className = 'absolute inset-0 flex items-center justify-center text-4xl text-green-400 font-bold pointer-events-none animate-[pulse_1s_ease-in-out_infinite]';
