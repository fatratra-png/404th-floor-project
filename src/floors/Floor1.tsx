      ...
      <div className="flex-1 flex p-6 gap-6">
        ...
        <div className="w-72 flex-shrink-0 flex flex-col gap-4">
          <div>
            ...
            <p className="text-slate-400 text-sm">Drag fuses to matching slots.</p>
          </div>
          <div className="flex-1 space-y-3 pr-2">
            ...
            {inventory.length === 0 && (
              <div className="text-slate-600 text-sm text-center py-8 font-mono">ALL FUSES INSERTED</div>
            )}
          </div>
          <div className="p-3 rounded bg-red-900/20 border border-red-900/50 flex items-start gap-2">
            <span className="material-symbols-outlined text-red-500 mt-0.5 text-sm">warning</span>
            <div>
              <div className="text-red-400 font-bold text-xs uppercase">Safety Warning</div>
              <p className="text-[10px] text-red-200/70">High voltage risk. Match fuse type to slot.</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div className="flex justify-between items-end">
            <div>
              <h1 className="text-2xl font-bold text-white glitch-text" data-text="MAIN BREAKER PANEL">
                MAIN BREAKER PANEL
              </h1>
              <p className="text-slate-400 text-sm">
                Status: <span className={completed ? 'text-green-500 font-mono font-bold' : 'text-red-500 font-mono font-bold'}>
          ...
          <div className="flex-1 relative bg-[#1a1f2c] rounded-xl border border-slate-600 p-6 overflow-hidden">
            <div
              className="absolute inset-0 opacity-20 pointer-events-none"
              style={{ backgroundImage: 'radial-gradient(#334155 1px, transparent 1px)', backgroundSize: '20px 20px' }}
            />
            <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-50">
              <path d="M 100 200 L 250 200 L 250 150" fill="none" stroke="#334155" strokeWidth={2} />
              <path d="M 100 210 L 240 210 L 240 300" fill="none" stroke="#334155" strokeWidth={2} />
              <path d="M 400 150 L 400 200 L 550 200" fill="none" stroke="#334155" strokeWidth={2} />
              <path d="M 700 300 L 700 210 L 850 210" fill="none" stroke="#334155" strokeWidth={2} />
            </svg>
...
function FuseCard({ fuse, onDrop }: { fuse: FuseItem; onDrop: (type: string, item: FuseItem) => void }) {
  ...
  return (
    <div
      ...
      className="group relative bg-slate-800/50 border border-slate-700 rounded-lg p-3 hover:border-primary/50 transition-all cursor-grab active:cursor-grabbing"
    >
      <div className={`absolute -left-1 top-1/2 -translate-y-1/2 w-1 h-8 rounded-full ${colorMap[fuse.color] || 'bg-yellow-500/10'}`} />
      <div className="flex items-center gap-3">
        <div className="w-10 h-14 bg-gradient-to-br from-slate-700 to-slate-900 rounded border border-slate-600 flex items-center justify-center relative overflow-hidden">
          <span className={`material-symbols-outlined text-${fuse.color} text-xl`}>{fuse.icon}</span>
        </div>
        <div>
          <div className="text-white font-bold tracking-widest text-sm">{fuse.label}</div>
          <div className={`text-xs font-mono text-${fuse.color}`}>{fuse.voltage}</div>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div
        ...
        onDrop={handleDrop}
        className={`relative w-24 h-32 border-2 border-dashed rounded-lg flex flex-col items-center justify-center transition-all ${
          filled
            ? 'border-green-500 bg-slate-900/80'
            : hover
              ? 'border-primary bg-slate-900/80 shadow-[0_0_20px_rgba(19,91,236,0.3)]'
              : `border-${colorAccent} bg-slate-900/50`
        }`}
      >
        {filled ? (
          <span className={`material-symbols-outlined text-3xl text-${colorAccent}`}>
            {slot.accept === 'thermal' ? 'whatshot' : slot.accept === 'quantum' ? 'blur_on' : 'electric_bolt'}
          </span>
        ) : (
          <>
            <span className="material-symbols-outlined text-slate-600 text-3xl mb-2">download</span>
            <span className="text-[10px] font-mono uppercase text-slate-500">{slot.label}</span>
          </>
        )}
      </div>
      <div className="flex items-center gap-2">
        <div className={`w-2 h-2 rounded-full ${filled ? 'bg-primary animate-pulse' : 'bg-slate-600'}`} />
        <span className={`text-xs font-mono ${filled ? 'text-primary font-bold' : 'text-slate-500'}`}>
